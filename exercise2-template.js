...
function threeStepsAB(text) {
  ...
  for (let i = 0; i < text.length; i++) {
    // Cek jika karakter saat ini adalah 'a'
    if (text[i] === "a") {
      // Cek apakah ada 'b' yang berjarak 3 karakter dari 'a'
      if (i + 4 < text.length && text[i + 4] === "b") {
        return true;
      }
    }
    // Cek jika karakter saat ini adalah 'b'
    if (text[i] === "b") {
      // Cek apakah ada 'a' yang berjarak 3 karakter dari 'b'
