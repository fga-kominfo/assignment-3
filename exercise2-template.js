...
function threeStepsAB(text) {
  // Ubah semua huruf menjadi huruf kecil untuk menghindari perbedaan besar kecil huruf
  text = text.toLowerCase();

  // Loop untuk mengecek setiap karakter dalam string
  for (let i = 0; i < text.length; i++) {
    // Cek apakah karakter 'a' diikuti 'b' dengan jarak 3 karakter atau sebaliknya
    if (
      (text[i] === "a" && text[i + 4] === "b") ||
      (text[i] === "b" && text[i + 4] === "a")
    ) {
