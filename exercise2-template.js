...
function threeStepsAB(text) {
   ...
   for (let i = 0; i < text.length; i++) {
    // Jika menemukan 'a'
    if (text[i] === 'a') {
        // Periksa apakah ada 'b' yang berjarak minimal 3 karakter setelah 'a'
        if (text[i + 4] === 'b') {
            return true;
        }
    }
    // Jika menemukan 'b'
    if (text[i] === 'b') {
        // Periksa apakah ada 'a' yang berjarak minimal 3 karakter setelah 'b'
        if (text[i + 4] === 'a') {
            return true;
        }
    }
}
// Jika tidak ditemukan pasangan 'a' dan 'b' yang memenuhi syarat return false
return false;
}
