...
function sumArray(arr, int) {
  ...
  // Iterasi pertama untuk memilih elemen pertama dari pasangan
  for (let i = 0; i < arr.length; i++) {
    // Iterasi kedua untuk mencari elemen kedua yang menjumlahkan target
    for (let j = i + 1; j < arr.length; j++) {
      if (arr[i] + arr[j] === int) {
        result.push([arr[i], arr[j]]);
        count++;
        if (count === 3) {
          // Membatasi output pada 3 pasangan
          return result;
        }
      }
    }
